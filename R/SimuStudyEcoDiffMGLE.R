#### ---- Simulation studies for MGLE on the EcoDiff model ---- ####
...
L <- parLapply(cl , seeds , 
               function(s){
                 ...
                 for(i in 1:N.sim){
                   ...
                   mgle <- optim( theta.0 ,
                                  function(theta){
                                    mom.capt <- Moments_EcoDiff(N = N ,d = d , 
                                                                sigma = theta[1] , 
                                                                v = theta[2:3],
                                                                p = theta[4] ,
                                                                t0 = t0 , t = t,
...
saveRDS( MGLE , paste(c("MGLE, EcoDiff, theta=(", 
                        theta.theo[1], "," ,
                        theta.theo[2], "," ,
                        theta.theo[3], "," ,
                        theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
#Saving the Gauss log-likelihood maximal values
saveRDS( LL.MGLE , paste(c("LL.MGLE, EcoDiff, theta=(", 
                           theta.theo[1], "," ,
                           theta.theo[2], "," ,
                           theta.theo[3], "," ,
                           theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
#Saving the Hessians at MGLE
saveRDS( MGLE.Hess , paste(c("MGLE.Hess, EcoDiff, theta=(", 
                             theta.theo[1], "," ,
                             theta.theo[2], "," ,
                             theta.theo[3], "," ,
                             theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
