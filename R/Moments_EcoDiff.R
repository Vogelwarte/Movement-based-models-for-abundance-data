...
Moments_EcoDiff <- function(N , 
                            t0 , t , d ,
                            p , sigma , v , 
                            Regions.B.L , dx){
  #Function to compute the mean vector and covariance matrix of the random matrix from an EcoDiff model wrapped into a vector following the R conventions.
  ...
  #               l$mean.vec <- the mean vector of a wrapped-into-a-vector random matrix from an EcoDiff model.                            
  #               l$cov.mat <- the covariance matrix of a wrapped-into-a-vector random matrix from an EcoDiff model.                            
  n.t <- length(t)
  ...
  l$mean.vec <- (N*p)*as.vector(U)
  l$cov.mat <- diag( (N*p)*as.vector(U) , nrow = n.s*n.t )
  return(l)
}