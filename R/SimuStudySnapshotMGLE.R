#### ---- Simulation studies for MGLE on the Snapshot model ---- ####
...
L <- parLapply(cl , seeds , 
               function(s){
                 ...
                 for(i in 1:N.sim){
                   Q.sim[ i , ] <- as.vector(
                     Sim_Snapshot(N = N ,d = d , 
                                 theta.X = theta.theo[1:3] ,
                                 p = theta.theo[4] ,
                                 t0 = t0 , t = t,
                   ...
                   mgle <- optim( theta.0 ,
                                  function(theta){
                                    mom.capt <- Moments_Snapshot(N = N ,d = d , 
                                                                theta.X = theta[1:3] ,
                                                                p = theta[4] ,
                                                                t0 = t0 , t = t,
                                                                Regions.B.L = Regions.B.L  , dx = dx )
...
saveRDS( Q.sim , paste(c("Q.sim, Snapshot, theta=(", 
                         theta.theo[1], "," ,
                         theta.theo[2], "," ,
                         theta.theo[3], "," ,
                         theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
#Saving the MGL punctual estimates
saveRDS( MGLE , paste(c("MGLE, Snapshot, theta=(", 
                        theta.theo[1], "," ,
                        theta.theo[2], "," ,
                        theta.theo[3], "," ,
                        theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
#Saving the Gauss log-likelihood maximal values
saveRDS( LL.MGLE , paste(c("LL.MGLE, Snapshot, theta=(", 
                           theta.theo[1], "," ,
                           theta.theo[2], "," ,
                           theta.theo[3], "," ,
                           theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
#Saving the Hessians at MGLE
saveRDS( MGLE.Hess , paste(c("MGLE.Hess, Snapshot, theta=(", 
                             theta.theo[1], "," ,
                             theta.theo[2], "," ,
                             theta.theo[3], "," ,
                             theta.theo[4], "), N=", N ,".rds"), collapse = "" ))
