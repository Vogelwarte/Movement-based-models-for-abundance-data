#### ---- MGLE for the EcoDiff model over the Rover fly data ---- ####

library(numDeriv)

source( "Gauss_ll.R"  )
source( "Moments_EcoDiff.R"   )
...
MGLE <- optim( theta0 ,
               function(theta){
                 mom.eco_diff <- Moments_EcoDiff(N=N , p=theta[4] , sigma = theta[1] ,
                                                 v = theta[1+1:d] , t0 = t0 , t = t , d = d ,
                                                 Regions.B.L = Regions.B.L , dx = dx) 
                 ll <- Gauss_ll( Q.obs ,
                                 m = mom.eco_diff$mean.vec,
                                 C = mom.eco_diff$cov.mat)
...
mom.MGLE <- Moments_EcoDiff(N=N , p=MGLE$par[4] , sigma = MGLE$par[1] ,
                            v = MGLE$par[1+1:d] , t0 = t0 , t = t , d = d ,
                            Regions.B.L = Regions.B.L , dx = dx)
start <- Sys.time()
